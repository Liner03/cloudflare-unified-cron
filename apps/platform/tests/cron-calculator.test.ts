...
} from "../src/infrastructure/cron/cron-calculator";
...
describe("CronCalculator", () => {
  it("is strictly after the base instant", () => {
    ...
    ).toBe(Date.parse("2026-09-08T00:05:00Z"));
  });
  ...
  it("uses Unix weekday numbering where 1 is Monday", () => {
    ...
    ).toBe(Date.parse("2026-09-07T00:00:00Z"));
  });
  ...
  it("crosses month, year, leap-day and day-31 boundaries", () => {
    expect(
      calculator.nextAfter(
        "59 23 31 12 *",
        "UTC",
        Date.parse("2026-12-31T23:58:00Z"),
      ),
    ).toBe(Date.parse("2026-12-31T23:59:00Z"));
    expect(
      calculator.nextAfter(
        "0 0 31 * *",
        "UTC",
    ...
    ).toBe(Date.parse("2026-05-31T00:00:00Z"));
    expect(
      calculator.nextAfter(
        "0 0 29 2 *",
        "UTC",
  ...
  it("locks cron-parser 5.10 DST behavior for New York in 2026", () => {
    expect(
      calculator.nextAfter(
        "30 2 * * *",
        "America/New_York",
        Date.parse("2026-03-07T07:30:00Z"),
      ),
    ).toBe(Date.parse("2026-03-08T07:30:00Z"));
    const fall = calculator.preview(
      "30 1 * * *",
      "America/New_York",
  ...
  it("fails within the bounded search window for impossible dates", () => {
    ...
    expect(() =>
      calculator.nextAfter("* * * * *", "Not/A_Timezone", Date.now()),
    ).toThrow();
  });

  it.each([0, 11, 1.5])("rejects preview count %s", (count) => {
    expect(() =>
      calculator.preview("* * * * *", "UTC", Date.now(), count),
    ).toThrow("预览数量必须为 1..10");
  });

  it("bounds expression and timezone input lengths", () => {
    expect(() => validateCronExpression("")).toThrow("长度");
    expect(() => validateCronExpression("1".repeat(129))).toThrow("长度");
    expect(() => calculator.nextAfter("* * * * *", "", Date.now())).toThrow(
      "时区长度",
    );
    expect(() =>
      calculator.nextAfter("* * * * *", "x".repeat(129), Date.now()),
    ).toThrow("时区长度");
  });

  it("normalizes harmless whitespace before parsing", () => {
