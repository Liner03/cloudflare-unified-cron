...
export async function executeIdempotentMutation(
  ...
  if (!key || !/^[A-Za-z0-9._:-]{8,128}$/.test(key)) {
    throw new ApiError(422, "IDEMPOTENCY_KEY_REQUIRED", "写操作需要 8..128 字符的 Idempotency-Key");
  }
  ...
  try {
    await context.env.DB.batch([
      context.env.DB
        ...
        .bind(scope, key, requestHash, plan.status, responseJson, now, now + IDEMPOTENCY_TTL_MS),
      plan.statement,
    ...
    if (isMutationConflict(error)) {
      throw new ApiError(409, plan.conflictCode, plan.conflictMessage);
    }
    throw error;
  }
...
async function readStored(db: D1Database, scope: string, key: string): Promise<StoredIdempotency | null> {
  return db
    .prepare(
...
function replay(stored: StoredIdempotency, requestHash: string): Response {
  if (stored.request_hash !== requestHash) {
    throw new ApiError(409, "IDEMPOTENCY_CONFLICT", "相同 Idempotency-Key 已用于不同请求体");
  }
...
function isMutationConflict(error: unknown): boolean {
  ...
  return /constraint failed|malformed JSON|UNIQUE constraint/i.test(error.message);
}
