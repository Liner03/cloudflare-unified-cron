...
### 触发架构修复追加本地回归（2026-09-12）

- [ ] `L2-023` 多 Target 配置生成一致，未知绑定和跨 Target 调用被拒绝。
- [ ] `L2-024` 配置为 100 条规则时注册、列表、更新与并发容量守卫一致。
- [ ] `L2-025` Queue 模式原子物化与重投保持 occurrence identity，已入队不记为业务成功。
- [ ] `L2-026` 10/25/50/100 条同分钟到期能批量投递；慢 Target 不阻塞其他 Target；RPC 保持独立调用预算。
- [ ] `L2-027` 接收后响应丢失、重复 Queue 消息和结果回报失败不重复业务副作用；消费者在平台调用结束后继续工作。
- [ ] `L2-028` 投递回报凭据仅能更新对应 Target/Delivery，幂等回报及迟到冲突被正确处理。
...
- 2026-09-14 容量修复：提交 `9bb9bec7af05532d69e5e5b63fa49400476b9090` 将共享 Queue producer 改为跨 Target 全局领取并按 100 条/批发送；默认总规则、物化和投递预算提升到 1,000。本地 `L2-032` 的 500 Target/500 occurrence 以 5 批完成；`R7-013` 远程 250/500 仍无证据，不得标记 PASS。
- 2026-09-14 本轮远程测试开始时尚缺：部署容量 migration、`R7-013` 远程 250/500、`R3-009`/`R7-006` 连续 60 分钟 soak、`R7-007` Registration 更新与暂停/恢复、`R7-011` 精确响应丢失、`R7-012` 在途/DLQ，以及当前版本回滚恢复。下条记录给出完成状态。
- 2026-09-14 远程收尾：上述容量部署、`R7-013`、`R3-009`、`R7-006..007`、`R7-011..012` 与 `R8-001..005` 已按当前共享 Queue 架构 PASS。Platform 已恢复当前 build、全局暂停、未来规则、主 Queue/DLQ backlog=0。下一步先保存报告并删除 A/B/C Test Target，再执行 mkfast `R9-001..005`；`R8-006..009` 随最终报告、凭据和保留资源处理完成。
